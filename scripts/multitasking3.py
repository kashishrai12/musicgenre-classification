...
class MultiTaskNoiseAugmentationModel(nn.Module):
    def __init__(self, input_dim, embedding_dim=128, num_classes=10):
        ...
        self.contrastive_head = nn.Identity()  # For contrastive loss, just use embedding
    ...
    def forward(self, x):
        ...
        return contrastive_out, class_out
...
def train_multitask(train_features, train_labels, model, config, alpha=0.5):
    ...
    for epoch in range(config['max_epochs']):
        ...
        for x1, x2, label in pair_loader:
            ...
            # For classification, use only x1 and its label
            # Find the indices of x1 in the original features to get the correct labels
            # Here, we assume x1 comes from train_features in order
            # If not, you may need to adjust this logic
            # We'll use a simple approach: cycle through train_labels
            class_labels = torch.tensor(train_labels[:len(x1)], dtype=torch.long).to(config['device'])

            optimizer.zero_grad()
            out1, class_out1 = model(x1)
            out2, _ = model(x2)
            contrastive_loss = contrastive_criterion(out1, out2, label)
            classification_loss = classification_criterion(class_out1, class_labels)
...
if __name__ == "__main__":
    ...
    # Train with multi-task loss (alpha=0.5 for equal weighting)
    train_multitask(X_train, y_train, model, config, alpha=0.3)

    # Freeze embedding layers before fine-tuning
    for param in model.embedding_layers.parameters():
        param.requires_grad = False

    # Fine-tune with classification loss (only classification head is updated)
