...
class ELUClassifier5_2(nn.Module):
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_features, train_labels)):
        ...
        fold_result = run_training(X_train, y_train, X_val, y_val, config, ELUClassifier5_2, experiment_name, genre_names)
    ...
    test_result = evaluate_model(train_features, train_labels, test_features, test_labels, config, ELUClassifier5_2, genre_names)
