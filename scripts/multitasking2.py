...
def create_pairs_dataset(features, labels, batch_size=32):
    ...
    for i in range(num_samples):
        for j in range(i + 1, num_samples):
            ...
            anchor_indices.append(i)  # Track anchor index
    ...
    dataset = TensorDataset(pair1, pair2, pair_labels, anchor_indices)
...
def train_multitask(train_features, train_labels, model, config, alpha=0.5):
    ...
    for epoch in range(config['max_epochs']):
        model.train()
        total_loss = 0.0
        for x1, x2, label, anchor_idx in pair_loader:
            x1, x2, label, anchor_idx = x1.to(config['device']), x2.to(config['device']), label.to(config['device']), anchor_idx.to(config['device'])
