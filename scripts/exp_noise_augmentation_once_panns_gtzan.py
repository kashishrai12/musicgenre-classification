...
def add_noise(data, noise_factor=0.1, batch_size=100):
    noisy_data = np.empty_like(data)
    for start in range(0, data.shape[0], batch_size):
        end = min(start + batch_size, data.shape[0])
        noise = np.random.normal(0, noise_factor, data[start:end].shape).astype(np.float32)
        noisy_data[start:end] = data[start:end] + noise
    return noisy_data
...
def main():
    ...
    train_data = np.load(r"D:\research_project\preprocessed_panns_gtzan\extracted_train_panns_gtzan.npz")
    train_features = train_data['features'].astype(np.float32)
    train_labels = train_data['labels']
    
    # Load testing data
    print("Loading testing data...")
    test_data = np.load(r"D:\research_project\preprocessed_panns_gtzan\extracted_test_panns_gtzan.npz")
    ...
    config = {
        ...
        'experiment_name': 'exp_noise_augmentation_once_panns_gtzan',
        'weight_decay': 0.0,
        'input_dim': train_features.shape[1]  # Use the original feature dimension
    }
    ...
    test_result = evaluate_model(augmented_features, augmented_labels, test_features, test_labels, config, NoiseAugmentationModel, genre_names)
