...
# Multi-task Model with four output heads
class MultiTaskNoiseAugmentationModel(nn.Module):
    def __init__(self, input_dim, embedding_dim=128, num_classes=10):
        ...
        self.classification_head_anchor = nn.Linear(embedding_dim, num_classes)   # For anchor
        self.classification_head_pos = nn.Linear(embedding_dim, num_classes)      # For positive
        self.classification_head_neg = nn.Linear(embedding_dim, num_classes)      # For negative

    def forward(self, x):
        embedding = self.embedding_layers(x)
        contrastive_out = self.contrastive_head(embedding)
        class_out_anchor = self.classification_head_anchor(embedding)
        class_out_pos = self.classification_head_pos(embedding)
        class_out_neg = self.classification_head_neg(embedding)
        return contrastive_out, class_out_anchor, class_out_pos, class_out_neg
...
def create_triplet_dataset(features, labels, batch_size=32, n_pos=14, n_neg=14):
    ...
    dataset = TensorDataset(anchor_t, positive_t, negative_t, anchor_indices, positive_indices, negative_indices)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    return dataloader

# Multi-task Training Loop (with three classification heads, using triplet loss)
def train_multitask_triplet(train_features, train_labels, model, config, alpha=0.3, w_anchor=1/3, w_pos=1/3, w_neg=1/3):
    ...
    for epoch in range(config['max_epochs']):
        model.train()
        total_loss = 0.0
        for anchor, positive, negative, anchor_idx, pos_idx, neg_idx in triplet_loader:
            ...
            neg_idx = neg_idx.to(config['device'])

            class_labels_anchor = torch.tensor(train_labels[anchor_idx.cpu().numpy()], dtype=torch.long).to(config['device'])
            class_labels_pos = torch.tensor(train_labels[pos_idx.cpu().numpy()], dtype=torch.long).to(config['device'])
            class_labels_neg = torch.tensor(train_labels[neg_idx.cpu().numpy()], dtype=torch.long).to(config['device'])

            optimizer.zero_grad()
            anchor_emb, class_out_anchor, _, _ = model(anchor)
            positive_emb, _, class_out_pos, _ = model(positive)
            negative_emb, _, _, class_out_neg = model(negative)
            triplet_loss = triplet_criterion(anchor_emb, positive_emb, negative_emb)
            classification_loss_anchor = classification_criterion(class_out_anchor, class_labels_anchor)
            classification_loss_pos = classification_criterion(class_out_pos, class_labels_pos)
            classification_loss_neg = classification_criterion(class_out_neg, class_labels_neg)
            classification_loss = (
                w_anchor * classification_loss_anchor +
                w_pos * classification_loss_pos +
                w_neg * classification_loss_neg
            )
...
# Test the Model (using anchor head for classification)
def test_model(test_loader, model, config):
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader:
            ...
            _, logits_anchor, _, _ = model(X_batch)
...
if __name__ == "__main__":
    ...
    # Train with multi-task triplet loss (alpha=0.3, equal weights for all heads)
    train_multitask_triplet(X_train, y_train, model, config, alpha=0.3, w_anchor=1/3, w_pos=1/3, w_neg=1/3)
