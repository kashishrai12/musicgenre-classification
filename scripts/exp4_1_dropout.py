...
class DropoutClassifier1(nn.Module):
...
def main():
    ...
    config = {
        ...
        'experiment_name': 'exp4_1_dropout',
        'weight_decay': 0.0
    }
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_features, train_labels)):
        ...
        fold_result = run_training(X_train, y_train, X_val, y_val, config, DropoutClassifier1, experiment_name, genre_names)
    ...
    test_result = evaluate_model(train_features, train_labels, test_features, test_labels, config, DropoutClassifier1, genre_names)
