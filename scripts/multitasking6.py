...
# Multi-task Training Loop (with two classification heads)
...
if __name__ == "__main__":
    ...
    # Run for different alpha values and collect test accuracies
    alphas = np.arange(0.1, 1.0, 0.1)
    test_accuracies = []

    for alpha in alphas:
        print(f"\nTraining with alpha={alpha:.1f}")
        # Re-initialize model for each alpha
        model = MultiTaskNoiseAugmentationModel(input_dim=input_dim, embedding_dim=128, num_classes=num_classes).to(config['device'])
        train_multitask(X_train, y_train, model, config, alpha=alpha)
        acc = test_model(test_loader, model, config)
        test_accuracies.append(acc)

    # Plot the results
    plt.figure(figsize=(8, 5))
    plt.plot(alphas, test_accuracies, marker='o')
    plt.xlabel('Alpha (Contrastive Loss Weight)')
