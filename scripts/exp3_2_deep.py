...
class DeepClassifier2(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        self.layers = nn.Sequential(
            nn.Linear(input_dim, 256),  # 2x of 128
            nn.ReLU(),
            nn.Linear(256, 128),  # 2x of 64
            nn.ReLU(),
            nn.Linear(128, 64),  # 2x of 32
            nn.ReLU(),
            nn.Linear(64, 32),  # 2x of 16 (assuming original was 16)
            nn.ReLU(),
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_features, train_labels)):
        ...
        fold_result = run_training(X_train, y_train, X_val, y_val, config, DeepClassifier2, experiment_name, genre_names)
    ...
    test_result = evaluate_model(train_features, train_labels, test_features, test_labels, config, DeepClassifier2, genre_names)
