...
def run_training_with_noise(X_train, y_train, X_val, y_val, config, model_class, experiment_name, genre_names):
    ...
    for epoch in range(config['max_epochs']):
        # Apply noise augmentation at the beginning of each epoch
        noisy_X_train = add_noise(X_train, noise_factor=0.1)
        
        # Training step
        model.train()
        optimizer.zero_grad()
        outputs = model(torch.tensor(noisy_X_train, dtype=torch.float32))
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_features, train_labels)):
        ...
        fold_result = run_training_with_noise(X_train, y_train, X_val, y_val, config, NoiseAugmentationModel, experiment_name, genre_names)
