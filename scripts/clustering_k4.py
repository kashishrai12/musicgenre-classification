...
def evaluate_model(model, X_test, y_test, genre_names, group=None):
    ...
    with torch.no_grad():
        ...
        # Ensure genre_names are strings
...
def main():
    ...
    # Step 1: K-Means Clustering with K=4
    ...
    percentages = df.groupby(['Genre', 'Cluster']).size().groupby(level=0).apply(lambda x: 100 * x / x.sum())
    print(f"\nCluster Percentages for K = {k}:\n{percentages}")
    percentages.to_csv(rf"D:\research_project\results\cluster_percentages_k{k}.csv")

    # Step 2: Design Hierarchical Classification System
    print("\nDesigning Hierarchical Classification System...")
    df_hierarchy = pd.DataFrame({'Genre': y_train, 'Group': clusters})
    group_mapping = df_hierarchy.groupby('Genre')['Group'].first().to_dict()
    print(f"Genre to Group Mapping:\n{group_mapping}")

    # Prepare group labels
    group_labels_train = np.array([group_mapping[genre] for genre in y_train])
    group_labels_test = np.array([group_mapping[genre] for genre in y_test])
    ...
    augmented_group_labels = np.concatenate((group_labels_train, group_labels_train), axis=0)
    
    # Cross-validation for group classifier
    config['num_classes'] = len(np.unique(group_labels_train))
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    group_fold_results = []
    
    for fold, (train_idx, val_idx) in enumerate(skf.split(augmented_X_train, augmented_group_labels)):
        print(f"\nGroup Classifier - Fold {fold + 1}/5")
        X_train_fold, X_val_fold = augmented_X_train[train_idx], augmented_X_train[val_idx]
        y_train_fold, y_val_fold = augmented_group_labels[train_idx], augmented_group_labels[val_idx]
        
        fold_result = run_training(X_train_fold, y_train_fold, X_val_fold, y_val_fold, config, NoiseAugmentationModel)
        group_fold_results.append(fold_result)
    
    # Train final group classifier on all data
    group_classifier = NoiseAugmentationModel(input_dim=X_train.shape[1], num_classes=len(np.unique(group_labels_train)))
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(group_classifier.parameters(), lr=config['learning_rate'], weight_decay=config['weight_decay'])
    
    for epoch in range(config['max_epochs']):
        group_classifier.train()
        optimizer.zero_grad()
        outputs = group_classifier(torch.tensor(augmented_X_train, dtype=torch.float32))
        loss = criterion(outputs, torch.tensor(augmented_group_labels, dtype=torch.long))
        loss.backward()
        optimizer.step()
    ...
    group_test_result = evaluate_model(group_classifier, X_test, group_labels_test, group_names)
    ...
    for group in np.unique(group_labels_train):
        group_indices_train = np.where(group_labels_train == group)[0]
        group_indices_test = np.where(group_labels_test == group)[0]
        ...
        # Get genre names for this group using the main label_encoder
        ...
        config['num_classes'] = len(np.unique(y_group_train_encoded))
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        genre_fold_results = []
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(augmented_X_group, augmented_y_group)):
            ...
            fold_result = run_training(X_train_fold, y_train_fold, X_val_fold, y_val_fold, 
        ...
        genre_classifier = NoiseAugmentationModel(input_dim=X_group_train.shape[1], 
                                               num_classes=len(np.unique(y_group_train_encoded)))
        criterion = nn.CrossEntropyLoss()
        ...
        genre_classifiers[group] = genre_classifier
        genre_results[group] = genre_test_result
    
    # Save performance metrics
    with open(r"D:\research_project\results\hierarchical_classification_results_k4.csv", 'w', newline='') as csvfile:
